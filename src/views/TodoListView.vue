<template>
  <ion-page>
    ...
    <ion-content class="overflow-auto">
      <div
        v-show="
          (currentCategory || categoryTitle === 'all' || categoryTitle === 'another') &&
          !isLoadingOpen
        ">
          ...
          <ion-toolbar class="text-center">
            <div style="height: 40px">
              <ion-icon
                ...
                color="primary"
                size="large"
                v-if="categoryTitle === 'all'"></ion-icon>
              <ion-icon
                :icon="icons.ellipsisHorizontalCircle"
                color="medium"
                size="large"
                v-else-if="categoryTitle === 'another'"></ion-icon>
              <template v-else>
                <ion-icon
                  size="large"
                  class="text-gray-300"
            ...
            <h2 class="text-2xl">{{ categoryTitle.toUpperCase() }} TODOS</h2>
          </ion-toolbar>
        </ion-header>
        <ion-list>
          <ion-list-header>
...
const { id: categoryId } = route.query;

const { todayTodos, lateTodos, laterTodos, doneTodos } = storeToRefs(useTodos());
const { getTodos, doneTodo, undoneTodo, deleteTodo } = useTodos();
...
onBeforeMount(async () => {
  ...
  await getTodos(categoryTitle as string);
